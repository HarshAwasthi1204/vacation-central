...
TipTrickIdRef = ForwardRef('TipTrickId')
TripIdRef = ForwardRef('TripId')
SearchHistoryIdRef = ForwardRef('SearchHistoryId')
TrendingTripIdRef = ForwardRef('TrendingTripId')
...
class ContactNo(BaseModel):
    # country_code: str = Field(pattern="^(\\+91)|(0?)$")
    country_code: str | None = Field(default="+91", pattern="^(\\+91)|(0?)$")
    # contact_no: str = Field(pattern="^[6-9][0-9]{9}$")
    contact_no: str
...
class TipTrickId(BaseModel):
    tip_trick_id: Annotated[Optional[ObjectId], ObjectIdPydanticAnnotation] = None

class TipTrick(TipTrickId):
    # tip_trick_id: str
    title: str
    # description: str
    author: UserIdRef # type: ignore
...
class SearchHistory(SearchHistoryId):
    user: UserIdRef # type: ignore
    search_query: str
    # search_result: list[UserIdRef] | list[PostIdRef] | list[CommentIdRef] | list[DiaryIdRef] | list[TipTrickIdRef] | None = None # type: ignore
    search_result: list[UserIdRef, PostIdRef, CommentIdRef, DiaryIdRef, TipTrickIdRef] | None = None # type: ignore

class TrendingTripId(BaseModel):
    trending_trip_id: Annotated[Optional[ObjectId], ObjectIdPydanticAnnotation] = None

class TrendingTrip(TrendingTripId):
    # trending_trip_id: str
...
class User(UserId):
    ...
    searchhistory : list[SearchHistoryIdRef] | None = None # type: ignore
    # region: str
    region: str
    gender: Gender
    # gender: str | None = None
    # aadhar_card: FileUrl | FilePath
    aadhar_card: FileUrl | FilePath | dict | None = None
    diaries: list[DiaryIdRef] | None = None # type: ignore
    posts: list[PostIdRef] | None = None # type: ignore
    tips_tricks: list[TipTrickIdRef] | None = None # type: ignore
    trending_trips: list[TrendingTripIdRef] | None = None # type: ignore
    # trips: list[TripIdRef] | None = None # type: ignore
    bio: str | None = None
